...
void __interrupt() ISR(void)
{
    ...
    {
        __delay_ms(30);
        // Update the servo position based on the current position
        if (currentPosition == 0)
        {
            currentPosition = 1; // Move to 0° position
            setServoPosition(45); // 0° position (1.5 ms pulse width)
        }
        else if (currentPosition == 1)
        {
            currentPosition = 2; // Move to +90° position
            setServoPosition(75); // +90° position (2 ms pulse width)
        }
        else if (currentPosition == 2)
        {
            currentPosition = 3; // Move back to 0° position
            setServoPosition(45); // 0° position (1.5 ms pulse width)
        }
        else if (currentPosition == 3)
        {
            currentPosition = 0; // Move back to -90° position
            setServoPosition(16); // -90° position (1 ms pulse width)
        }
